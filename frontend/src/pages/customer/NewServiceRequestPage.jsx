...
// Static service catalog — company's fixed offerings
const services = [
  { id: 'maapi', name: 'Apply for Mapi', icon: '📐', price_range: '₹2,000 – ₹8,000' },
  { id: 'division', name: 'Apply for Batwara', icon: '🗂️', price_range: '₹5,000 – ₹15,000' },
  { id: 'map', name: 'Apply for Map', icon: '🗺️', price_range: '₹3,000 – ₹10,000' },
  { id: 'tools', name: 'Apply for All Amin Tools', icon: '🧰', price_range: 'As per tools' },
];
...
export default function NewServiceRequestPage() {
  ...
  useEffect(() => {
    const fetchDistricts = async () => {
      try {
        const res = await api.get('/public/districts');
        setDistricts((res.data.districts || []).map(d => d.name));
      } catch (_) {
        // Fallback districts
        setDistricts(['Madhepura', 'Patna', 'Bhagalpur', 'Purnia', 'Araria', 'Kishanganj', 'Katihar', 'Saharsha']);
      }
    };
    fetchDistricts();
  }, []);
