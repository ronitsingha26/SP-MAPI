...
export default function BantwaraApplicationsPage() {
  ...
  useEffect(() => {
    api.get('/applications').then(res => {
      setApps((res.data.applications || []).filter(a => a.service_type === 'bantwara'));
    }).catch(err => {
  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <div>
          <h1 className="page-title flex items-center gap-2"><GitBranch className="w-5 h-5 text-yellow-600" /> Bantwara Applications</h1>
          <p className="page-subtitle">Your land division survey requests</p>
        </div>
        <Link to="/customer/services/bantwara" className="btn-primary text-sm"><Plus className="w-4 h-4" /> Apply New</Link>
      </div>
      <div className="card">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <Link to="/customer/services/bantwara" className="btn-primary mt-4">Apply for Bantwara</Link>
          </div>
        ) : (
