...
import { User, Phone, Mail, Lock, Eye, EyeOff, MapPin, Upload, CheckCircle2, ChevronRight, ChevronLeft, FileText, Leaf, LogIn, Copy, Check } from 'lucide-react';
...
// ── Beautiful Post-Submit Success Screen ──────────────────────────────────────
function SuccessScreen({ appId, userMobile, userEmail, userPassword, onLoginSuccess }) {
  const { login } = useAuth();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setLoginError(err.response?.data?.message || err.message || 'Login failed. Try again.');
    } finally {
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-brand-green to-emerald-500 px-8 py-10 text-center relative overflow-hidden">
            <div className="absolute inset-0 opacity-20">
              ...
              <div className="absolute bottom-4 right-8 w-24 h-24 border-2 border-white rounded-full" />
            </div>
          ...
          <div className="px-8 py-6 bg-brand-green-pale/50 border-b border-brand-green-pale">
            ...
            <div className="flex items-center gap-3 bg-white rounded-2xl px-5 py-4 shadow-sm border border-brand-green-light">
              ...
              <button
                onClick={copyAppId}
                className="p-2 rounded-xl bg-brand-green-pale hover:bg-brand-green hover:text-white text-brand-green transition-all"
            ...
            <p className="text-center text-xs text-brand-text-muted mt-2">
              💡 Save this ID to track your application status anytime
            </p>
          </div>
          ...
          <div className="px-8 py-6">
            ...
            {loginDone ? (
              <div className="text-center py-4">
                ...
                <p className="text-brand-green font-semibold">Logged in! Redirecting to your portal…</p>
              </div>
            ) : (
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <FormField label="Mobile Number or Email" required>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Mobile or email used during registration"
                      value={loginField}
                ...
                >
                  ...
                  {loginLoading ? 'Logging in…' : 'Login & View My Application'}
                </button>
              </form>
            )}

            <p className="text-center text-xs text-brand-text-muted mt-4">
              You can also login later at{' '}
              <Link to="/login" className="text-brand-green font-semibold hover:underline">Customer Portal</Link>
            </p>
          </div>
        </div>
      </div>
...
export default function MapiRegistrationPage() {
  ...
  const [successData, setSuccessData] = useState(null); // { appId, mobile, email, password }
  const [serviceConfig, setServiceConfig] = useState(null);

  const [form, setForm] = useState({
    name: '', mobile: '', email: '', password: '', confirmPassword: '',
    fatherName: '', state: 'Bihar', district: '', panchayat: '',
    policeStation: '', village: '', wardName: '', moujaName: '', khataNumber: '', blockName: '', pincode: '',
    landArea: '',
    aadhaarFront: null, aadhaarBack: null, landDocument: null,
  });

  useEffect(() => {
    // Fetch dynamic pricing for mapi
    api.get('/services').then(res => {
      const mapiService = res.data?.services?.find(s => s.name === 'mapi');
      if (mapiService) setServiceConfig(mapiService);
    }).catch(console.error);
  }, []);

  const calculatePrice = () => {
    if (!serviceConfig) return form.landArea ? (Number(form.landArea) * 3).toLocaleString() : '0';
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Register customer if not logged in (or if logged in as Admin)
      ...
      if (!registeredUser) {
        try {
          ...
          if (e.response?.status === 409) {
            // Already registered - try to login with provided password
            try {
      ...
      // 2. Submit Mapi Application via FormData
      const formData = new FormData();
      if (registeredUser?.id) formData.append('customer_id', registeredUser.id);
      ...
      // Show the beautiful success screen (instead of auto-redirect)
      setSuccessData({
  ...
  if (successData) {
    return (
      <SuccessScreen
        ...
        userMobile={successData.mobile}
        userEmail={successData.email}
        userPassword={successData.password}
      />
    );
  }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <FormField label="Email Address">
                  <div className="relative">
                    ...
                    <input className="input pl-10" type="email" placeholder="Optional" value={form.email} onChange={e => set('email', e.target.value)} />
                  </div>
                </FormField>
            ...
            <div className="space-y-5 animate-fade-in">
              ...
              <div className="mt-4 p-4 bg-brand-green-pale rounded-xl flex justify-between items-center border border-brand-green">
                <span className="font-semibold text-brand-text">
                  Estimated Price {serviceConfig ? `(Base: ₹${serviceConfig.base_price || 0}${serviceConfig.unit_type !== 'fixed' ? ` + ₹${serviceConfig.unit_price}/${serviceConfig.unit_type}` : ''})` : '(₹3 / sq.ft)'}:
                </span>
            ...
            <div className="space-y-5 animate-fade-in">
              ...
              <div className="mt-6 p-4 bg-brand-yellow-pale rounded-2xl text-sm text-brand-text">
                ...
                <ul className="text-brand-text-muted space-y-1 list-disc list-inside">
                  <li>A customer account will be created with your mobile/email</li>
                  <li>A unique Application ID will be generated</li>
                  <li>You can login with mobile/email + password to track status</li>
                </ul>
              </div>
            </div>
          )}
