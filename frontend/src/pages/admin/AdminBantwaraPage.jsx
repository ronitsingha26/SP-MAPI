...
import { GitBranch, Filter, Search, RefreshCw, ChevronDown, UserCheck, Trash2 } from 'lucide-react';
...
export default function AdminBantwaraPage() {
  ...
  const fetchApps = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ service_type: 'bantwara', limit: 100 });
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title flex items-center gap-2">
            <GitBranch className="w-6 h-6 text-yellow-600" /> Bantwara Applications
          </h1>
          <p className="page-subtitle">Review and manage land division survey requests</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <select className="input py-2 bg-brand-green-pale border-none" value={filter} onChange={e => setFilter(e.target.value)}>
              ...
              {BANTWARA_STATUSES.map(s => <option key={s} value={s}>{STATUS_LABEL[s]}</option>)}
            </select>
          </div>
        </div>
      </div>
          ...
          <div className="table-wrapper">
            <table className="data-table">
              ...
              <tbody>
                {apps.map(app => (
                  <>
                    ...
                    >
                      ...
                      <td className="text-sm text-brand-text-muted">{app.district || '—'}</td>
                      ...
                      <td>
                        <div className="flex items-center gap-1">
                          <button
                            className="p-1.5 hover:bg-brand-green-pale rounded-lg text-brand-green"
                            onClick={(e) => {
                              e.stopPropagation();
                ...
                {apps.length === 0 && (
                  <tr><td colSpan="7" className="text-center py-12 text-brand-text-muted">No Bantwara applications found</td></tr>
                )}
              </tbody>
            </table>
          </div>
