...
import { ClipboardList, Filter, Search, UserCheck } from 'lucide-react';
...
export default function AdminMapiPage() {
  ...
  const fetchApplications = async () => {
    const data = await applicationService.getApplicationsByType('mapi');
    setApps(data);
  };
  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <div>
          <h1 className="page-title flex items-center gap-2"><ClipboardList className="w-6 h-6 text-brand-green" /> Manage Mapi Applications</h1>
          <p className="page-subtitle">Review and update land measurement requests</p>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <select className="input py-2 bg-brand-green-pale border-none" value={filter} onChange={e => setFilter(e.target.value)}>
              ...
              {Object.entries(STATUS_CONFIG.mapi).map(([key, cfg]) => (
            ...
            <tbody>
              {filteredApps.map(app => (
                <tr key={app.appId}>
                  ...
                  <td>
                    {(() => {
                      const cfg = STATUS_CONFIG.mapi[app.status] || { label: app.status, color: 'badge-grey' };
                  ...
                  <td>
                    <div className="flex items-center gap-2">
                      ...
                      <button className="p-1 hover:bg-brand-green-pale rounded text-brand-green" title="Assign Amin">
