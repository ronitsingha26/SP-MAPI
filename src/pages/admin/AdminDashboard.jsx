import { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { Users, UserCheck, Wrench, CreditCard, ArrowRight, TrendingUp, AlertCircle } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts';
import { serviceRequests as mockServiceRequests, customers, dashboardStats } from '../../data/index';
import { formatCurrency } from '../../utils/helpers';
import { getAllServiceRequests } from '../../utils/storage';

const stats = dashboardStats.admin;

function StatCard({ icon: Icon, label, value, sub, color, to }) {
  return (
    <Link to={to} className="card-hover p-6">
      <div className="flex items-start justify-between mb-3">
        <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${color === 'green' ? 'bg-brand-green-pale' : 'bg-brand-yellow-pale'}`}>
          <Icon className={`w-5 h-5 ${color === 'green' ? 'text-brand-green' : 'text-yellow-600'}`} />
        </div>
        <ArrowRight className="w-4 h-4 text-brand-text-muted opacity-0 group-hover:opacity-100 transition-opacity" />
      </div>
      <p className="text-2xl font-bold text-brand-text">{value}</p>
      <p className="text-sm text-brand-text-muted mt-0.5">{label}</p>
      {sub && <p className="text-xs text-brand-green font-medium mt-1">{sub}</p>}
    </Link>
  );
}

export default function AdminDashboard() {
  const [serviceRequests, setServiceRequests] = useState([]);
  
  useEffect(() => {
    // Merge real data from localStorage with the mock data for demonstration
    const localRequests = getAllServiceRequests();
    // Map local requests to match the mock data schema for the table
    const formattedLocal = localRequests.map(req => ({
      id: req.appId,
      customer_name: req.name,
      service_type: req.serviceType,
      district: req.district || 'N/A',
      created_at: new Date(req.createdAt).toLocaleDateString('en-IN'),
      status: req.status,
      amin_name: req.assignedAminName || null
    }));
    setServiceRequests([...formattedLocal, ...mockServiceRequests]);
  }, []);

  const pendingServices = serviceRequests.filter(s => s.status === 'pending' || s.status === 'submitted');

  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <div>
          <h1 className="page-title">Admin Dashboard</h1>
          <p className="page-subtitle">District: Patna, Araria, Ramgarh</p>
        </div>
        <span className="badge-green">✓ Active Admin</span>
      </div>

      {/* Stat Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard icon={Users} label="Total Customers" value={stats.total_customers} color="green" to="/admin/customers" />
        <StatCard icon={UserCheck} label="Active Amins" value={stats.active_amins} sub="In the field" color="green" to="/admin/amins" />
        <StatCard icon={Wrench} label="Pending Services" value={stats.pending_services} sub="Needs Amin assignment" color="yellow" to="/admin/services" />
        <StatCard icon={CreditCard} label="District Revenue" value={formatCurrency(stats.district_revenue)} color="green" to="/admin/payments" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        {/* Monthly Applications Chart */}
        <div className="card p-6">
          <h2 className="font-bold text-brand-text mb-5">Monthly Applications</h2>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={stats.monthly_applications} barSize={28}>
              <CartesianGrid strokeDasharray="3 3" stroke="#E8F5EE" />
              <XAxis dataKey="month" tick={{ fontSize: 12, fill: '#6B7C6D' }} />
              <YAxis tick={{ fontSize: 12, fill: '#6B7C6D' }} />
              <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #A8D5B5', background: '#fff' }} />
              <Bar dataKey="count" fill="#4CAF82" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        {/* Pending Alerts */}
        <div className="card p-6">
          <h2 className="font-bold text-brand-text mb-4 flex items-center gap-2">
            <AlertCircle className="w-5 h-5 text-yellow-500" /> Action Required
          </h2>
          <div className="space-y-3">
            {pendingServices.length === 0 ? (
              <p className="text-sm text-brand-text-muted py-8 text-center">No pending service assignments.</p>
            ) : (
              pendingServices.slice(0, 4).map(s => (
                <div key={s.id} className="flex items-center justify-between p-3 bg-brand-green-pale rounded-xl">
                  <div>
                    <p className="text-sm font-medium text-brand-text capitalize">{s.service_type} — {s.district}</p>
                    <p className="text-xs text-brand-text-muted">By {s.customer_name} · Needs Amin</p>
                  </div>
                  <Link to="/admin/services" className="btn-primary py-1.5 px-3 text-xs">Assign</Link>
                </div>
              ))
            )}
          </div>
        </div>
      </div>

      {/* Recent Activity Table */}
      <div className="card !p-0 overflow-hidden">
        <div className="px-6 py-4 border-b border-brand-green-pale flex items-center justify-between">
          <h2 className="font-bold text-brand-text">Recent Service Applications</h2>
          <Link to="/admin/services" className="text-sm text-brand-green font-semibold hover:underline flex items-center gap-1">
            View All <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
        <div className="table-wrapper">
          <table className="data-table">
            <thead>
              <tr>
                <th>Request ID</th>
                <th>Customer</th>
                <th>Service Type</th>
                <th>District</th>
                <th>Submitted</th>
                <th>Status</th>
                <th>Assigned Amin</th>
              </tr>
            </thead>
            <tbody>
              {serviceRequests.slice(0,5).map(s => (
                <tr key={s.id}>
                  <td className="font-mono text-xs">{s.id}</td>
                  <td className="font-medium">{s.customer_name}</td>
                  <td className="capitalize text-xs font-medium text-brand-text-muted">{s.service_type}</td>
                  <td className="text-xs">{s.district}</td>
                  <td className="text-xs text-brand-text-muted">{s.created_at}</td>
                  <td>
                    <span className={s.status === 'completed' ? 'badge-green' : s.status === 'pending' ? 'badge-yellow' : 'bg-blue-50 text-blue-600 px-3 py-1 rounded-full text-xs font-semibold'}>
                      {s.status}
                    </span>
                  </td>
                  <td className="text-xs font-medium">
                    {s.amin_name ? s.amin_name : <span className="text-yellow-600 font-semibold">Pending Assign</span>}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
