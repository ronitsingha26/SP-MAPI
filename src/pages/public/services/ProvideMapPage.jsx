...
export default function ProvideMapPage() {
  ...
  const [form, setForm] = useState({
    ...
    // Map-specific
    thanaNumber: '', sheetNumber: '', landType: '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (!currentUser && (!form.email || !form.password)) { setError('Email and password required to create an account'); return; }
    ...
    try {
      ...
      saveApplication('map', {
        ...
        thanaNumber: form.thanaNumber, sheetNumber: form.sheetNumber, landType: form.landType,
        status: 'submitted', remark: '', statusHistory: [], mapDownloadUrl: null,
        documents: { aadhaarFront: form.aadhaarFront?.name, aadhaarBack: form.aadhaarBack?.name },
      });
      setSuccessId(appId);
        ...
        <form onSubmit={handleSubmit} className="card p-8 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {!currentUser && (<>
                <FormField label="Email Address" required><input className="input" type="email" placeholder="your@email.com" value={form.email} onChange={e => set('email', e.target.value)} /></FormField>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <FormField label="Panchayat"><input className="input" placeholder="Panchayat" value={form.panchayat} onChange={e => set('panchayat', e.target.value)} /></FormField>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField label="Thana Number"><input className="input" placeholder="Thana number" value={form.thanaNumber} onChange={e => set('thanaNumber', e.target.value)} /></FormField>
              <FormField label="Sheet Number"><input className="input" placeholder="Sheet number" value={form.sheetNumber} onChange={e => set('sheetNumber', e.target.value)} /></FormField>
              <FormField label="Land Type">
                <select className="input" value={form.landType} onChange={e => set('landType', e.target.value)}>
