...
export default function Navbar() {
  ...
  const servicesDropdown = [
    { label: 'Apply for MAPI', to: '/services/mapi', icon: ClipboardList },
    { label: 'Apply for Batwara', to: '/services/bantwara', icon: GitBranch },
    { label: 'Apply for Map', to: '/services/provide-map', icon: Map },
    { label: 'Apply for all Amin Tools', to: '/under-construction', icon: Wrench },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <div className="relative" ref={dropdownRef}
              ...
              <button
                ...
                className={`flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-150 ${isActive('/services') ? 'bg-brand-green-pale text-brand-green font-semibold' : 'text-brand-text-muted hover:bg-brand-green-pale hover:text-brand-green'}`}
              >
                    ...
                    {servicesDropdown.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-start gap-3 p-3 rounded-xl hover:bg-brand-green-pale transition-colors group ${location.pathname === item.to ? 'bg-brand-green-pale' : ''}`}
                        >
                          <div className={`p-1.5 rounded-lg ${location.pathname === item.to ? 'bg-brand-green text-white' : 'bg-gray-50 text-brand-text-muted group-hover:bg-brand-green group-hover:text-white transition-colors'}`}>
                            <Icon className="w-4 h-4" />
                          </div>
                          <div className="flex items-center h-full pt-1">
                            <p className={`text-sm font-semibold ${location.pathname === item.to ? 'text-brand-green' : 'text-brand-text group-hover:text-brand-green'}`}>
                              {item.label}
                            </p> </div>
                        </Link>
                      );
          ...
          <div className="hidden md:flex items-center gap-2">
              ...
              <div className="relative" ref={userMenuRef}>
                ...
                {userMenuOpen && (
                  <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-2xl shadow-card border border-brand-green-pale p-2 animate-fade-in">
                    <Link to="/admin/dashboard" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 w-full p-2 text-sm text-brand-text hover:bg-brand-green-pale hover:text-brand-green rounded-xl transition-colors font-medium">
            ...
            <button onClick={() => setAppointmentModalOpen(true)} className="btn-primary">
              <ClipboardList className="w-4 h-4" /> Book Appointment
            </button>
          </div>
